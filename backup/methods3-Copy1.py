...
def pred_data2(data, test_family, benchs_names):
    ...
    X2_train = torch.tensor(X2_train).double()
    ...
    X2_test = np.array(data_test.loc[:,['logS','logS*logT','logS^2', 'logS^2*logT']]) #
    X2_test = torch.tensor(X2_test).double()
    ...
    return X_train, X2_train, F_train, D_train, Y_train, X_test, X2_test, F_test, D_test, Y_test
...
def train_pca_regression_model(X_train, D_train, Y_train, X_test, D_test, Y_test, d):
    ...
    return np.abs(Y_test-Y_hat).cpu().detach().numpy().T
...
def train_single_index_model(X_train, D_train, Y_train, X_test, D_test, Y_test, benchs_names, kernel, h, d, num_epochs=10000, patience = 500, tol=1e-3, device='cpu'):
    ...
    return torch.abs(torch.vstack(Y_tests) - torch.vstack(Y_hats)).cpu().detach().numpy()
...
def train_basic_logistic_model(X_train, Y_train, X_test, Y_test, benchs_names, num_epochs=10000, patience = 500, tol=1e-3, device='cpu'):
    ...
    Y_hats = []
    Y_tests = []
    #print(X_train.shape, X_test.shape, beta.shape, alpha.shape)
    for j, _ in enumerate(benchs_names): 
...
def train_basic_intercept_logistic_logit_model(X_train, D_train, Y_train, X_test, D_test, Y_test, benchs_names):
    lower_bounds = {}
    for s in benchs_names:
        lower_bounds[s]=pd.read_csv('data/lower_bounds.csv')[s].iloc[0]
    Z_train = np.hstack((X_train.numpy(),D_train.numpy()))
    Z_test = np.hstack((X_test.numpy(),D_test.numpy()))
    Y_train_logit = np.hstack([logit(Y_train.numpy()[:,j], c=lower_bounds[s])[:,None] for j,s in enumerate(benchs_names)])
    
    reg = LinearRegression(fit_intercept=False).fit(Z_train, Y_train_logit)
    Y_hat_logit = reg.predict(Z_test)
    Y_hat = np.hstack([(lower_bounds[s]+(1-lower_bounds[s])*sigmoid(Y_hat_logit[:,j], use_torch=False))[:,None] for j,s in enumerate(benchs_names)])

    assert Y_test.shape==Y_hat.shape
    return np.abs(Y_hat - Y_test.numpy()).T
    
def run_results(test_family, data, benchs_names):
    X_train, X2_train, F_train, D_train, Y_train, X_test, X2_test, F_test, D_test, Y_test = pred_data2(data, test_family, benchs_names)

    pca_error = train_pca_regression_model(F_train, D_train, Y_train, F_test, D_test, Y_test, d)
    logistic_basic_error = train_basic_logistic_model(F_train, Y_train, F_test, Y_test, benchs_names)
    logistic_basic_intercept_error = train_basic_intercept_logistic_model(F_train, D_train, Y_train, F_test, D_test, Y_test, benchs_names)
    logistic_basic_intercept_logit_error = train_basic_intercept_logistic_logit_model(F_train, D_train, Y_train, F_test, D_test, Y_test, benchs_names)
    
    logistic_basic_interaction_error = train_basic_logistic_model(X_train, Y_train, X_test, Y_test, benchs_names)
    logistic_basic_interaction_intercept_error = train_basic_intercept_logistic_model(X_train, D_train, Y_train, X_test, D_test, Y_test, benchs_names)
    logistic_basic_interaction2_intercept_error = train_basic_intercept_logistic_model(X2_train, D_train, Y_train, X2_test, D_test, Y_test, benchs_names)
    logistic_basic_interaction2_logit_intercept_error = train_basic_intercept_logistic_logit_model(X2_train, D_train, Y_train, X2_test, D_test, Y_test, benchs_names)
    
    #single_index_error = train_single_index_model(F_train, D_train, Y_train, F_test, D_test, Y_test, benchs_names, kernel, h, d)
    #single_index_interaction_error = train_single_index_model(X_train, D_train, Y_train, X_test, D_test, Y_test, benchs_names, kernel, h, d)
    #single_index_interaction2_error = train_single_index_model(X2_train, D_train, Y_train, X2_test, D_test, Y_test, benchs_names, kernel, h, d)
    
    out = (pca_error,
           logistic_basic_error,
           logistic_basic_intercept_error,
           logistic_basic_intercept_logit_error,
           logistic_basic_interaction_error,
           logistic_basic_interaction_intercept_error,
           logistic_basic_interaction2_intercept_error,
           logistic_basic_interaction2_logit_intercept_error)
